...
public sealed class AccountAvatarManager(
    ...
    ILogger<AccountAvatarManager> logger)
{
    ...
    public async Task<AccountMutationResult> UploadDiscordAvatarAsync(
        int userId,
        Stream image,
        CancellationToken cancellationToken)
    {
        ...
        {
            return AccountMutationResult.Invalid("file", "The Discord avatar is not a recognized image.");
        }
        catch (SixLabors.ImageSharp.InvalidImageContentException)
        {
            return AccountMutationResult.Invalid("file", "The Discord avatar is corrupted or exceeds the allowed limits.");
        }
        ...
        logger.LogInformation("Deleted existing custom avatars for user {UserId}.", userId);
        await avatarStorage.UploadAvatarAsync(userId, normalized, cancellationToken);
        logger.LogInformation("Uploaded Discord avatar for user {UserId}.", userId);
        if (await accountRepository.SetAvatarAsync(userId, $"{userId}.webp", PictureType.Discord, cancellationToken))
        {
            logger.LogInformation("Set Discord avatar database value for user {UserId}.", userId);
            return AccountMutationResult.Success();
        }
