...
import { useUploadNpcImage } from '../queries'
...
const uploadMutation = useUploadNpcImage()
...
async function save() {
  if (!props.npcId || !cropperRef.value) return
  error.value = ''
  ...
  try {
    await uploadMutation.mutateAsync({ npcId: props.npcId, file: blob })
    toast.success('NPC picture updated.')
    emit('uploaded')
    setOpen(false)
  } catch (err) {
    ...
    <DialogPortal>
      ...
      >
        <div class="mb-4 flex items-start justify-between gap-4">
          <DialogTitle class="text-lg font-semibold">Crop NPC picture</DialogTitle>
          <DialogClose
            aria-label="Close"
        ...
        <p class="mb-3 text-sm text-muted-foreground">
          Drag to position and use the wheel or pinch to zoom. The image is locked to a square and
          will be saved as 256×256 webp.
        </p>
